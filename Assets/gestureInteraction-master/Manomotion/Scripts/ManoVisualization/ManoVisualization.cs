...
public class ManoVisualization : MonoBehaviour
{
   ...
   // [SerializeField]
   // private Transform bounding_box_prefab; //手部邊界框

   // [SerializeField]
   // private bool _show_bounding_box; //手部邊界框是否顯示

    //Not the AR Background but a Plane Mesh filled with the camera info from ManoMotion
    [SerializeField]
    private bool _show_background; //是否顯示背景

    [SerializeField]
    private Camera cam; //相機

    [SerializeField]
    private GameObject manomotionGenericLayer; //用於表示Manomotion的通用層
    ...
    //    {
    //        Debug.LogError("DAD WE Don't have boundingBox");
    //    }
    //}
